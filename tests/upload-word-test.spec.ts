...
test(
  'Family Vault - Word upload, download, and cleanup',
  async ({ page }) => {
    const loginPage =
      new LoginPage(page);
    ...
    // -----------------------------------------
    // Step 4: Check whether Word document exists
    // -----------------------------------------
    ...
    console.log(
      `Word document matches before setup: ${matchCount}`,
    );
    ...
    if (matchCount === 0) {
      console.log(
        'Word document not found — uploading fixture...',
      );
      ...
      console.log(
        `Word document matches after upload: ${matchCount}`,
      );
    } else {
      console.log(
        'Word document already exists — upload skipped.',
      );
    }

    // -----------------------------------------
    // Step 6: Verify exactly one Word document
    // -----------------------------------------
    ...
    console.log(
      'Exactly one Word document exists: PASS',
    );
    ...
    if (testDocumentCreated) {
      expect(
        afterUploadCount,
        'Document count did not increase by exactly one after Word upload',
      ).toBe(
        initialVaultCount + 1,
      );
    ...
    if (testDocumentCreated) {
      ...
      console.log(
        `Word deletion: ${testFileName} — PASS`,
      );
      ...
      console.log(
        `Word document matches after cleanup: ${afterCleanupMatchCount}`,
      );
      ...
      console.log(
        'Existing Word document preserved: PASS',
      );
    }
