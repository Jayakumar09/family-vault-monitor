...
test(
  'Family Vault - Excel upload, download, and cleanup',
  async ({ page }) => {
    const loginPage =
      new LoginPage(page);
    ...
    // -----------------------------------------
    // Step 4: Check whether Excel document exists
    // -----------------------------------------
    ...
    console.log(
      `Excel document matches before setup: ${matchCount}`,
    );
    ...
    if (matchCount === 0) {
      console.log(
        'Excel document not found — uploading fixture...',
      );
      ...
      console.log(
        `Excel document matches after upload: ${matchCount}`,
      );
    } else {
      console.log(
        'Excel document already exists — upload skipped.',
      );
    }

    // -----------------------------------------
    // Step 6: Verify exactly one Excel document
    // -----------------------------------------
    ...
    console.log(
      'Exactly one Excel document exists: PASS',
    );
    ...
    if (testDocumentCreated) {
      expect(
        afterUploadCount,
        'Document count did not increase by exactly one after Excel upload',
      ).toBe(
        initialVaultCount + 1,
      );
    ...
    if (testDocumentCreated) {
      ...
      console.log(
        `Excel deletion: ${testFileName} — PASS`,
      );
      ...
      console.log(
        `Excel document matches after cleanup: ${afterCleanupMatchCount}`,
      );
      ...
      console.log(
        'Existing Excel document preserved: PASS',
      );
    }
